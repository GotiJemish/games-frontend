...
export function GameGate({ config, loading, children }: GameGateProps) {
  // Non-blocking: render children immediately while loading.
  // Only block if config has loaded AND game is explicitly disabled.
  if (!loading && config && !config.is_public) {
    return (
