...
export default function ChessAIPage() {
  ...
  const connectWebSocket = (gId: string, uName: string) => {
    ...
    const wsUrl = `${wsProto}//${cleanHost}/chess/${gId}/ws?username=${encodeURIComponent(uName)}`;
  ...
  const handlePlayVsComputer = async (e: React.FormEvent) => {
    ...
    const botCol = myCol === "WHITE" ? "BLACK" : "WHITE";
    
    try {
      const createRes = await api.post(`chess/createLobby`, {
        username: uName,
        color: myCol,
        game_type: "chess",
        difficulty
      ...
      await api.post(`chess/${gId}/add_bot`, {
        username: `Computer (Bot) ${botCol}`,
        color: botCol
      });
      
      const startRes = await api.post(`chess/${gId}/start`);
      ...
      {!isJoined ? (
        <div className="w-full max-w-md bg-white dark:bg-zinc-900/80 backdrop-blur-md border border-zinc-200 dark:border-zinc-800 rounded-3xl p-8 shadow-xl dark:shadow-2xl relative z-10 transition-colors duration-300">
          <div className="text-center mb-6">
            ...
            <p className="text-zinc-500 dark:text-zinc-400 text-sm mt-1">Single Player Chess Bot Match</p>
          </div>
          ...
          <form onSubmit={handlePlayVsComputer} className="space-y-6">
            ...
            <div className="flex gap-3">
              ...
              <button
                type="submit"
                className="flex-[2] bg-gradient-to-r from-indigo-600 to-purple-600 hover:opacity-95 text-white font-bold rounded-xl py-3.5 flex items-center justify-center gap-2 cursor-pointer shadow-lg active:scale-98 transition-all"
              >
        ...
        <div className="w-full max-w-6xl grid grid-cols-1 lg:grid-cols-12 gap-8 relative z-10">
            ...
            <div className="w-full max-w-[560px] flex items-center justify-between px-2 text-sm text-zinc-500 dark:text-zinc-400">
              <div className="flex items-center gap-2">
                ...
                <span className="font-semibold text-zinc-850 dark:text-zinc-200">
                  {game?.players.find(p => p.color === (perspective === "WHITE" ? "BLACK" : "WHITE"))?.username || "Computer (Bot)"}
                </span>
          ...
          <div className="lg:col-span-4 flex flex-col space-y-6">
            <div className="bg-white dark:bg-zinc-900/60 border border-zinc-200 dark:border-zinc-800 rounded-3xl p-6 shadow-lg backdrop-blur-md">
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-lg font-extrabold text-zinc-900 dark:text-white flex items-center gap-2">
                  <Award className="w-5 h-5 text-indigo-500" /> AI Bot Match
                </h2>
                <button
              ...
              <div className="space-y-3.5 text-sm">
                ...
                <div className="flex justify-between border-b border-zinc-150 dark:border-zinc-850 pb-2">
                  ...
                  <span className="capitalize font-bold text-violet-400">{game?.board_state?.difficulty}</span>
                </div>
                {game?.winner && (
            ...
            <div className="bg-white dark:bg-zinc-900/60 border border-zinc-200 dark:border-zinc-800 rounded-3xl p-6 shadow-lg backdrop-blur-md flex flex-col h-[280px]">
              <h2 className="text-sm font-extrabold uppercase tracking-wider text-zinc-500 dark:text-zinc-400 mb-3 flex items-center gap-2">
                <MessageSquare className="w-4 h-4 text-indigo-500" /> Chat with Bot
              </h2>
                ...
                {chatMessages.filter(m => m.type === "chat").length === 0 ? (
                  ...
                  chatMessages.filter(m => m.type === "chat").map((chat, idx) => (
                    <div key={idx} className="space-y-0.5">
                      ...
                      <p className="bg-zinc-50 dark:bg-zinc-950 p-2.5 rounded-2xl rounded-tl-none border border-zinc-150 dark:border-zinc-850/50 text-zinc-800 dark:text-zinc-200">
                        {chat.message}
                      </p>
                    </div>
                  ))
