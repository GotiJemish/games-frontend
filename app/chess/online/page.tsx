...
import {
  ...
  MessageSquare, AlertCircle, Sparkles, Award, Globe, Copy, Check, Users
} from "lucide-react";
...
export default function ChessOnlinePage() {
  ...
  const joinLobby = async (e: React.FormEvent) => {
    ...
    try {
      const joinRes = await api.post(`chess/${gId}/joinLobby`, {
        username: uName,
        color: myCol
      });
  ...
  const handleStartGame = async () => {
    if (!gameId) return;
    try {
      const startRes = await api.post(`chess/${gameId}/start`);
